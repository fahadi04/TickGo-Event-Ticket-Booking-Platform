...
import com.fahad.TickGo.exceptions.*;
...
public class EventServiceImpl implements EventService {
    ...
    public Event updateEventForOrganizer(UUID organizerId, UUID id, UpdateEventRequest event) {
        ...
        existingEvent.setSalesStart(LocalDateTime.parse(event.getSalesStart()));
        existingEvent.setSalesEnd(LocalDateTime.parse(event.getSalesEnd()));
        existingEvent.setStatus(event.getStatus());

        Set<UUID> requestTicketTypeIds = event.getTicketTypes()
                .stream()
                .map(UpdateTicketTypeRequest::getEventId)
        ...
        for (UpdateTicketTypeRequest ticketType : event.getTicketTypes()) {
            if (null == event.getEventId()) {
            ...
            } else if (existingTicketTypesIndex.containsKey(ticketType.getEventId())) {
                //Update
                TicketType existingTicketType = existingTicketTypesIndex.get(ticketType.getEventId());
                ...
                throw new TicketTypeNotFoundException(
                        String.format("Ticket type with ID '%s' does not exists", ticketType.getEventId())
                );
            }
        }
